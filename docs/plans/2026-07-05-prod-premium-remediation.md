...
- 2026-07-06: Wave G2 landed b2633918 (zod on contents PUT/DELETE with base64 cap + sha create-vs-update; issues labels-replace ACCEPTS empty clear-all array — deliberate relaxed variant; assignees login-list; webhook update; workflow dispatch scalar inputs; strict empty-body ping/sync; community-health generate/commit-fix bounded while KEEPING domain codes invalid_file_type/invalid_body; 41-case suite; 3 test mocks gained new export names). 147 tests green. Server body-validation sweep now COMPLETE.
- 2026-07-06: Wave G3 landed 9b13021a (color-contrast 104→0 across the 9 scanned views: muted slate text stepped 400→500/600 on light, solid pills -500→-700, danger reds deepened incl. central Button soft-danger, issue-label chips derive AA text per theme via new issueLabelColors util; checkA11y gained settleAnimations — capped rAF stability poll — so contrast reads final colours not mid-fade blends; color-contrast REMOVED from DEFERRED_SERIOUS_RULES and hard-gated; only scrollable-region-focusable remains deferred). Verified independently: axe 9/9, 439 unit tests across 75 mirror files. Known pre-existing DARK-mode contrast debt reported by agent (footer slate-500/slate-950, ThemeOption labels, visibility toggle) — gate scans light mode; dark pass is a future item.
- 2026-07-06: Wave H0 landed f0d697e6 by lead (layout spec W1+W3: --layout-px ladder 16/24/32/40/48 + --layout-max-w 2400px consumed by App/Header/3 banners; @theme named breakpoints nav/wide/ultra replacing raw min-[Npx]; --card-min-width 340px ultra step → 2560 displays now 4 columns + 80px gutters, was 3 cols + 320px dead; anti-drift lint gate min-[Npx]/shell max-w + 5-case test; CSS-cascade source-order bug caught+fixed live at 1440). Verified live 1300/1440/1920/2560.
- 2026-07-06: Wave H1 landed 2dae79c7 (dark-mode contrast zeroed: LegalFooter slate-500→400, SettingsModal ThemeOption + visibility toggle; a11y spec now scans light AND dark — 18 hard-gated scans via VIEWS array + localStorage theme seed). TransferModal same-pattern fix landed 3b827b4b by lead.
- 2026-07-06: Wave H2 landed 06f69971 (duration-300 → motion tokens in 21 files by rule fast/slow; src has ZERO duration-300; lint gate gained the duration-300 rule + test; prod build verified).
- 2026-07-06: Wave H3 landed 9902d91f (SlimSidebar popovers real: full sidebarProps flow; ActionHistoryRow/ActivityRow/QuickActionButtons extracted ONCE and shared by expanded + slim; count pills on rail icons folded into aria-labels; per-popover accessible names; last-5 rows + honest empties; TDD-first mirror, 21 tests). Layout spec 2026-06-25 now FULLY EXECUTED (W1 ultrawide, W2 SlimSidebar, W3 tokens+gate, W4 done earlier by G3).
- 2026-07-06: Wave I (docs + final-product) landed — b5bfda8f CHANGELOG [4.5.0] + version bump (193 commits since 4.4.0 summarized); 761c7390 RepoCard content-visibility perf (22/30 cards skip paint at 1240px) + revived the permanently-self-skipping context-menu-scroll-free e2e spec (0/3 → 3/3 real assertions); e1e53583 hero screenshots refreshed at v4.5.0 UI; 4196b3e8 core docs to truth (README counts/claims, architecture off deleted .sql files onto ledger v28, tier table from feature-flags.js ground truth — old table claimed SSO which is false on all tiers, /live vs /ready, G10 validation layer); ad23ad32 API reference sweep ~120→~310 endpoints, all 24 TOC anchors were broken, migration Pro-gating claim was false (quota-metered at execute), GDPR/marks/health/envelopes sections added. Owner still needs to cut the v4.5.0 tag for a GitHub Release (README links to CHANGELOG anchor meanwhile).
