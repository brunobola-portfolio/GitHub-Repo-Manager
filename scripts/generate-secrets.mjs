...
export function appendSecretsToFile(target, secrets, io = fs) {
  ...
  if (!existing) {
    // Create with owner-only permissions. appendFileSync on a NEW file uses
    // 0666 & ~umask (0644 typically), i.e. world-readable — for a file whose
    // entire content is secrets. No-op on Windows, which ignores mode.
    io.writeFileSync(target, block, { mode: 0o600 })
  } else {
