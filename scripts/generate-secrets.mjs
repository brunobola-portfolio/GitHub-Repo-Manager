...
export function appendSecretsToFile(target, secrets, io = fs) {
  const existing = io.existsSync(target) ? io.readFileSync(target, 'utf8') : ''
  ...
  if (!existing) {
    // 'wx' + mode, not writeFileSync: exclusive create is atomic, so nothing
    // can slip a file (or a symlink to one) in between the existsSync check
    // above and this write. It also fixes the permissions problem —
    // appendFileSync on a NEW file uses 0666 & ~umask, typically 0644, i.e.
    // world-readable, for a file whose entire content is secrets. The mode is
    // a no-op on Windows, which ignores it.
