...
const STAGES = [
  {
    ...
    items: [
      ...
      { title: 'Bitbucket Migration Importer' },
      { title: 'Advanced Analytics Dashboard', description: 'Commit heatmaps, contributor insights, dependency graph' },
      { title: 'Dependency Graph Visualizer', description: 'Interactive graph of repo dependencies (SBOM-derived)' },
      { title: 'SSO / SAML', description: 'Single sign-on via your identity provider (Okta, Entra ID, SAML 2.0)', tier: 'Enterprise' },
      { title: 'Backup & Restore System', description: 'Scheduled snapshots with point-in-time restore' },
      { title: 'Security Alerts Dashboard', description: 'Aggregated Dependabot and secret scanning alerts' },
      { title: 'SBOM Export', description: 'Software Bill of Materials in SPDX / CycloneDX format', tier: 'Enterprise' },
      { title: 'Release Notes Generator', description: 'AI-generated changelogs from commit history' },
    ],
  },
  {
    id: 'later',
    items: [
      { title: 'GitHub Enterprise Server', description: 'Self-hosted GitHub Enterprise support' },
      { title: 'Plugin / Extension System', description: 'Third-party integrations via a public plugin API' },
      { title: 'Mobile App (React Native)', description: 'iOS and Android companion app' },
      { title: 'Org Permissions Sync', description: 'Mirror team memberships across orgs automatically' },
      { title: 'Dependabot Aggregation', description: 'Unified view of all dependency alerts across repos' },
      { title: 'Custom Workflow Templates', description: 'Reusable GitHub Actions templates per org' },
    ],
  },
  {
    id: 'shipped',
    items: [
      ...
      { title: 'Custom AI Model Selection', description: 'Choose provider + model per feature via Settings → AI Configuration (covered by BYOK)', tier: 'All' },
      { title: 'README Enhance (AI diff)', description: 'AI-suggested improvements over your current README', tier: 'All' },
      { title: 'Batch Indexing', description: 'Bulk AI indexing with progress modal', tier: 'All' },
      { title: 'DORA metrics + CSV export', description: 'Deploy frequency, lead time p50/p90, change failure rate, MTTR — with CSV export. Free on every tier, no webhook tier gate', tier: 'All' },
      { title: 'Azure DevOps Server (on-premise)', description: 'The migration wizard accepts any Azure DevOps host, cloud or self-hosted 2018+, via ALLOWED_AZURE_HOSTS', tier: 'All' },
      ...
      { title: 'AI Issue-to-PR Planner', description: 'Plan-only mode — structured plan (approach, files, tests, risks, estimate) for any open issue', tier: 'All' },
      ...
      { title: 'CODEOWNERS Parser + Generator', description: 'Parse existing CODEOWNERS OR suggest rules from recent commit authorship (hotspots + per-path owners + copy-to-clipboard preview)', tier: 'All' },
      { title: 'Security Posture report card', description: '10-check security report card (branch protection, secret scanning, Dependabot, workflow permissions, org 2FA) with an AI top-actions summary — plus the underlying Dependabot + secret scanning alert aggregation', tier: 'All' },
      { title: 'Compare with Existing — side-by-side diff', description: 'From the Similar Repositories drawer, open a side-by-side README / package.json diff against any similar repo', tier: 'All' },
    ],
  },
]
