    ...
    return (
      <div className="flex items-center justify-center h-full min-h-0 flex-1 bg-white dark:bg-slate-950">
        <div className="text-center space-y-3">
          ...
          <button
            onClick={() => onBack?.()}
            className="px-4 py-2 text-sm font-medium rounded-lg transition-colors ds-brand-solid ds-focus-ring"
          >
            Go back
          </button>
        </div>
    ...
    <div className="flex flex-col h-[100dvh] bg-white dark:bg-slate-950">
      ...
      {!composerOpen && (
        <button
          ...
          className="group lg:hidden fixed z-[var(--ds-z-floating)] bottom-[calc(152px+env(safe-area-inset-bottom,0px))] right-0 mr-4 w-14 h-14 rounded-full ds-brand-solid dark:hover:bg-[color:var(--ds-accent-brand)] ds-elevation-lg flex items-center justify-center transition-transform duration-[var(--ds-duration-slow)] ease-[var(--ds-ease-emphasized)] translate-x-[55%] opacity-90 hover:translate-x-0 hover:opacity-100 focus:translate-x-0 focus:opacity-100 active:translate-x-0"
          aria-label={
