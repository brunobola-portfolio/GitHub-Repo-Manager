...
export function DiffComputeOnDemand({ filename, additions = 0, deletions = 0, children }) {
    ...
    return (
        <div className="diff-compute-on-demand p-6 text-center bg-amber-50/60 dark:bg-amber-900/10 border border-dashed border-amber-300 dark:border-amber-800/60 rounded-lg m-3">
            <p className="font-mono text-[11px] text-slate-400 dark:text-slate-500 mb-1 truncate" title={filename}>
                {filename}
            </p>
            ...
            <div className="mt-4">
                <button
                    ...
                    className="inline-flex items-center gap-1 px-3 py-1.5 max-md:min-h-11 text-sm font-semibold rounded-lg bg-amber-600 text-white shadow-sm shadow-amber-600/30 hover:bg-amber-700 motion-safe:active:scale-95 transition-all"
                >
