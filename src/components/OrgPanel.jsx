...
export function OrgPanel({
    ...
    return (
        <Card className="p-4 space-y-4">
            ...
            {/* Create Hint */}
            {showCreateHint && (
                <div className="p-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-lg text-sm text-amber-800 dark:text-amber-200">
            ...
            <div className="space-y-1">
                <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-lg transition-all ${
                        selectedOrg === null
                            ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-200 border-2 border-indigo-200 dark:border-indigo-700 shadow-sm'
                            : 'hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border-2 border-transparent'
                    }`}
                >
                    {user?.avatar_url && (
                ...
                {orgs.map(org => (
                    <div key={org.login} className="relative group">
                        ...
                        <button
                            ...
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md opacity-0 group-hover:opacity-100 hover:bg-slate-200 dark:hover:bg-slate-600 transition-all"
                        >
                            <MoreVertical className="w-4 h-4 text-slate-500 dark:text-slate-400" />
                        </button>

                        {showOrgMenu === org.login && (
                            <div className="absolute right-0 top-full mt-1 w-40 bg-white dark:bg-slate-800 rounded-lg shadow-lg dark:shadow-slate-900/50 border border-slate-200 dark:border-slate-700 py-1 z-10">
                                <button
