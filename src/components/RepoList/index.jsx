				...
				<>
					{selectedIds.size > 0 && !sheetOpen && (
						<button
							...
							className="fixed bottom-[calc(56px+1rem+var(--safe-area-inset-bottom,0px))] left-1/2 -translate-x-1/2 z-[var(--ds-z-popover)] flex items-center gap-2 px-4 min-h-11 bg-slate-900/90 text-white rounded-full ds-elevation-overlay ds-focus-ring"
						>
							<span className="text-sm font-medium">{selectedIds.size} selected</span>
							<span className="text-xs">— tap for actions</span>
						</button>
					)}
					<SelectionSheet
						isOpen={sheetOpen}
