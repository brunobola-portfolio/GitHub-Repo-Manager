 ...
 *   - On-premises TFS / ADS  → host = user-entered (e.g. "tfs.empresa.com")
 *
    ...
    return (
      <div className="rounded-2xl border border-indigo-300 dark:border-indigo-700 bg-indigo-50/50 dark:bg-indigo-900/15 p-3 space-y-2.5">
        <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
          ...
          Indica o hostname do teu servidor on-premises (sem <code className="px-1 rounded bg-slate-200 dark:bg-slate-700">https://</code>):
        </div>
        <div className="flex items-stretch gap-2">
          <input
            ...
            placeholder="ex: tfs.empresa.com  ou  tfs.empresa.com:8080"
          ...
          <button
            ...
            onClick={commit}
            disabled={!draft.trim()}
            className="px-3 py-2 text-xs font-semibold rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:cursor-not-allowed transition-colors"
          >
            Guardar
          </button>
          <button
        ...
        {!host && (
          ...
          >
            <div className="px-3.5 pb-2.5 text-xs text-slate-500 dark:text-slate-400 border-t border-slate-200/40 dark:border-slate-700/40 pt-2">
              Para começar: cola uma URL do teu Azure DevOps / TFS no campo do topo,
              ou clica <strong>Cloud</strong> / <strong>On-prem</strong> aqui à direita.
            </div>
          </motion.div>
        )}
      </AnimatePresence>
